document.addEventListener('DOMContentLoaded', () => {
    ...
    const applyBackgrounds = () => {
        Object.keys(backgrounds).forEach(key => {
            ...
            // Special case for dashboard blur
            if (key === 'dashboard') {
    ...
    const updateBackgroundData = (key, newValues) => {
        ...
        if (!backgrounds[key].url && (backgrounds[key].blur === undefined || backgrounds[key].blur === 0)) {
    ...
    // Event Delegation for the new settings page
    if (dom.backgroundSettingsScreen) {
        ...
        // Handle URL and Blur Inputs
        dom.backgroundSettingsScreen.addEventListener('input', (e) => {
