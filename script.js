document.addEventListener('DOMContentLoaded', () => {
    // A global object to hold all DOM element references
    const dom = {};
    ...
    // --- Helper Functions (can be used by any page) ---
    ...
    // --- Page Initializers ---
    // This function runs the logic for index.html
    const initializeHomePage = () => {
        // Find all necessary elements for this page
        Object.assign(dom, {
            ...
            characters.forEach(char => {
                ...
                if (isBatchDeleteMode) {
                    ...
                    item.addEventListener('click', () => { 
                        ...
                        sessionStorage.setItem('activeCharacterId', activeCharacterId); // Persist across reloads
        ...
        // Batch Delete
        window.enterBatchDeleteMode = () => { isBatchDeleteMode = true; dom.homeScreen.classList.add('batch-delete-active'); renderCharacterList(); };
        window.exitBatchDeleteMode = () => { isBatchDeleteMode = false; dom.homeScreen.classList.remove('batch-delete-active'); renderCharacterList(); };
        ...
        dom.chatForm.addEventListener('submit', async (e) => {
            ...
            if (!apiUrl || !apiKey) { alert('请先在“我的”页面配置 API！'); window.location.href = 'dashboard.html'; return; }
    ...
    // This function runs the logic for dashboard.html
    const initializeDashboardPage = () => {
        ...
        const updateSliderProgress = (slider) => { const min = +slider.min; const max = +slider.max; const val = +slider.value; const percentage = (val - min) * 100 / (max - min); slider.style.setProperty('--progress', `${percentage}%`); };
        const applyWidgetStyles = (opacity, brightness) => { const root = document.documentElement; root.style.setProperty('--widget-bg-lightness', `${brightness}%`); root.style.setProperty('--widget-bg-alpha', opacity); };
        const saveWidgetSettings = () => { const settings = { opacity: dom.opacitySlider.value, brightness: dom.brightnessSlider.value, }; localStorage.setItem('aiChatWidgetSettings', JSON.stringify(settings)); };
        ...
        // Event Listeners
        document.querySelectorAll('.back-button').forEach(btn => btn.addEventListener('click', () => showScreen('myDashboard')));
        ...
        dom.iconTheme.addEventListener('click', (e) => { if (e.target.classList.contains('theme-swatch')) { e.stopPropagation(); alert('主题功能正在开发中！'); } });
        ...
        const onSliderInput = (event) => { applyWidgetStyles(dom.opacitySlider.value, dom.brightnessSlider.value); updateSliderProgress(event.target); };
    ...
    // --- SCRIPT ENTRY POINT ---
    // Check which page we are on and run the appropriate setup
    if (document.getElementById('home-screen')) {
        initializeHomePage();
    } else if (document.getElementById('my-dashboard-screen')) {
